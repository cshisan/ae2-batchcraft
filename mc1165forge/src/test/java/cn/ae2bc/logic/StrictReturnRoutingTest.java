...
public final class StrictReturnRoutingTest {
    ...
    public void outputCapabilityAndExtractionUseTheActiveBatchFilter() throws Exception {
        ...
        ClassNode endpoint = readClass(
                "/cn/ae2bc/part/PatternP2PTunnelPart$EndpointHandler.class");
        ...
        assertTrue(hasCallNamed(method(endpoint, "insertItem"), "returnOutputProduct")
                || hasCallNamed(method(endpoint, "insertItem"), "access$"));
    }
    ...
    public void everyUnitReturnPathUsesTheManagersDynamicFilter() throws Exception {
        ...
        assertTrue(countCallsNamed(method(port, "breakAdjacent"), "returnProduct") >= 2);
    }
    ...
    private static boolean hasCallNamed(MethodNode method, String name) {
        for (AbstractInsnNode instruction = method.instructions.getFirst();
            ...
            if (instruction instanceof MethodInsnNode) {
                String callName = ((MethodInsnNode) instruction).name;
                if (name.endsWith("$") ? callName.startsWith(name) : name.equals(callName)) return true;
            }
        }
        return false;
    }
