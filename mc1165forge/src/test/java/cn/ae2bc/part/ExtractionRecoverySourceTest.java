...
public final class ExtractionRecoverySourceTest {
    ...
    public void failedSourceRestoreIsPersistedAndDroppedWithThePart() throws Exception {
        ...
        assertTrue(tunnel.contains("extractionRecovery.read(data, EXTRACTION_RECOVERY)"));
        assertTrue(tunnel.contains("extractionRecovery.write(data, EXTRACTION_RECOVERY)"));
        ...
        assertTrue(queue.contains("entry.putLong(\"Amount\""));
    ...
    public void recoveryCanDrainAfterTaskEndButNewExtractionCannotStart() throws Exception {
        ...
        String outputTick = section(tunnel, "public TickRateModulation tickingRequest", "public boolean acceptsPlans");
        assertTrue(outputTick.indexOf("drainExtractionRecovery()")
                < outputTick.indexOf("!returnBatch.isActive()"));
        assertTrue(outputTick.contains("!extractionEnabled || !returnBatch.isActive()"));
