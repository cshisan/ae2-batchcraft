...
class PatternP2PUnitPortCollectSourceTest {
    ...
    @Test
    void fluidCollectionUsesTheCommonFilteredReturnPath() throws Exception {
        String source = Files.readString(Path.of(
                "src/main/java/cn/ae2bc/part/PatternP2PUnitPortPart.java"));
        int fluidMethod = source.indexOf("private boolean collectSourceFluid");
        int handler = source.indexOf("handleCollected(manager, what, amount, mode)", fluidMethod);
        int commonFilter = source.indexOf("if (!allowsInputFilter(what))", handler);

        assertTrue(fluidMethod >= 0 && handler > fluidMethod && commonFilter > handler);
    }
}
