...
管理器始终拥有本地配置值。开启同步时，输入端广播会覆盖返回、提取间隔和数量、单端口单槽、传输、破坏、红石、脉冲以及单元能源端口分配设置；关闭同步时忽略广播并继续使用本地值，再次开启同步会立即应用当前输入端配置。

“单元端口（能量）”控制管理器收到 FE 后向所属能源端口进行的第二级分配；即使管理器没有任务，该设置仍会生效。
