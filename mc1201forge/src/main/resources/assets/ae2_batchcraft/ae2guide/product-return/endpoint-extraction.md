...
The switch therefore controls only whether normal Outputs have extraction capability. The interval and amount affect both endpoint types. A Unit Extraction Port still requires its Manager's task to be operational, and a normal Output also needs an active, runnable task for new extraction. Unblocked return mode does not remove this requirement.
...
The recovery queue may continue draining after the task has ended. It contains only resources already pulled for return; it does not permit a new extraction after task completion.
