...
public final class PatternP2PUnitManagerPart extends CablePart implements PatternTaskEndpoint {
    ...
    private static final IPartModel MODEL = new PartModel(
            new ResourceLocation(Ae2bcMod.MOD_ID, "part/p2p/pattern_p2p_unit_manager"),
            new ResourceLocation(Ae2bcMod.MOD_ID, "part/pattern_p2p_unit_manager_glass"),
            new ResourceLocation(Ae2bcMod.MOD_ID, "part/pattern_p2p_unit_manager_frequency"));
    ...
    private @Nullable ForgeBlockCapabilityCache<GenericInternalInventory> returnTargetCache;
    ...
    private boolean handlePartUse(Player player, InteractionHand hand, Vec3 pos, boolean alternateUse) {
        ...
        if (alternateUse) {
            ...
            memoryCard.setMemoryCardContents(heldItem, getPartItem().asItem().getDescriptionId(), data);
            ItemData.setUnitId(heldItem, patternP2PUnitId);
            ...
            if (!ModContent.isPatternP2PFrequencySource(storedType) || !ItemData.hasP2PFrequency(data)) {
                ...
                if (logic.hasTaskState()) {
                    ...
                    setFrequency(ItemData.getP2PFrequency(data));
    ...
    public void exportSettings(SettingsFrom mode, CompoundTag builder) {
        ...
        if (mode == SettingsFrom.MEMORY_CARD) {
            builder.putString("p2pType", IPartItem.getId(getPartItem()).toString());
            if (frequency != 0) {
                ItemData.setP2PFrequency(builder, frequency);
            }
        }
    }
    ...
    private @Nullable GenericInternalInventory findInputReturnInventory() {
        ...
        if (returnTargetCache == null || returnTargetCache.level() != level
            ...
            returnTargetCache = ForgeBlockCapabilityCache.create(
                    appeng.capabilities.Capabilities.GENERIC_INTERNAL_INV, level, targetPos, targetSide);
        }
