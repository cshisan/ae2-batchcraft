...
public final class PatternP2PTunnelEnergyPart extends EnergyAcceptorPart {
    ...
    private ForgeBlockCapabilityCache<IEnergyStorage> sourceEnergyCache;
    ...
    protected double getFunnelPowerDemand(double maxRequired) {
        ...
        return Math.min(maxRequired, localDemand + PowerUnits.FE.convertTo(PowerUnits.AE, remoteDemand));
    }
    ...
    protected double funnelPowerIntoStorage(double power, Actionable mode) {
        ...
        try {
            ...
            double acceptedAe = PowerUnits.FE.convertTo(PowerUnits.AE, acceptedFe);
    ...
    private int getPullDemandFe() {
        double maxAe = PowerUnits.FE.convertTo(PowerUnits.AE, Integer.MAX_VALUE);
    ...
    private PullOutcome pullEnergyFromAdjacent() {
        ...
        if (sourceEnergyCache == null
            ...
            sourceEnergyCache = ForgeBlockCapabilityCache.create(
    ...
    private static int aeToFeFloor(double amount) {
        double converted = PowerUnits.AE.convertTo(PowerUnits.FE, Math.max(0, amount));
    ...
    private final class PullTicker implements IGridTickable {
        ...
        public TickingRequest getTickingRequest(IGridNode node) {
            return new TickingRequest(1, PULL_INTERVAL, !pullEnabled, true, PULL_INTERVAL);
        }
    ...
    private final class DynamicEnergyStorage implements IEnergyStorage {
        ...
        public int receiveEnergy(int maxReceive, boolean simulate) {
            ...
            double overflow = injectExternalPower(PowerUnits.FE, maxReceive,
                    simulate ? Actionable.SIMULATE : Actionable.MODULATE);
            int accepted = (int) cn.ae2bc.logic.Numbers.clamp(maxReceive - overflow, 0, maxReceive);
