...
/** Builds the six-face frequency and unit-identity indicators missing from AE2 rv6. */
final class PatternP2PUnitManagerIndicators {
    ...
    static List<BakedQuad> getQuads(PatternP2PUnitManagerPart manager) {
        long frequency = manager.getRenderFlag().longValue();
    ...
    private static List<BakedQuad> build(long frequency, long unit) {
        TextureAtlasSprite texture = Minecraft.getMinecraft().getTextureMapBlocks()
                .getAtlasSprite("appliedenergistics2:parts/p2p_tunnel_frequency");
        CubeBuilder builder = new CubeBuilder(DefaultVertexFormats.BLOCK);
