...
public final class PatternDispatchMetadata {
    ...
    static PatternDispatchMetadata create(IPatternDetails pattern) {
        try {
            ...
            Map<AEKey, Long> outputs = new LinkedHashMap<>(patternOutputs.length);
    ...
    private static MaterialOutputConfigData getMaterialOutputConfig(IPatternDetails pattern) {
        ...
        return ItemData.getMaterialOutput(processingPattern.getDefinition().toStack());
    }
    ...
    private static boolean hasExplicitDirections(IPatternDetails pattern, InputDirectionData directions) {
        ...
        for (int slot = 0; slot < sparseInputs.length && InputDirectionData.isValidSlot(slot); slot++) {
            if (sparseInputs[slot] != null && directions.getDirection(slot) != null) {
                return true;
            }
        }
        return false;
